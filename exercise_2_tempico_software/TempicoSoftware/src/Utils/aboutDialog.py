...
class Ui_AboutDialog(object):
    def setupUi(self, AboutDialog):
        ...
        # --- About Me: Personal experience section ---
        self.aboutMeFrame = QFrame(AboutDialog)
        self.aboutMeFrame.setObjectName(u"aboutMeFrame")
        self.aboutMeFrame.setFrameShape(QFrame.StyledPanel)
        self.aboutMeFrame.setFrameShadow(QFrame.Raised)
        self.verticalLayout_aboutme = QVBoxLayout(self.aboutMeFrame)
        self.verticalLayout_aboutme.setObjectName(u"verticalLayout_aboutme")

        self.aboutMeTitle = QLabel(self.aboutMeFrame)
        self.aboutMeTitle.setObjectName(u"aboutMeTitle")
        self.aboutMeTitle.setText("<b>About Me: A Positive Programming Experience</b>")
        self.verticalLayout_aboutme.addWidget(self.aboutMeTitle)

        self.aboutMeLabel = QLabel(self.aboutMeFrame)
        self.aboutMeLabel.setObjectName(u"aboutMeLabel")
        self.aboutMeLabel.setWordWrap(True)
        self.aboutMeLabel.setText(
            """
                <p>The project I remember most fondly is one I built during my Electronic \
                Engineering degree at Universidad Surcolombiana: an RFID-based asset \
                inventory system running on a Raspberry Pi. I had already worked with \
                Arduino and programmed microcontrollers in Micro C, but this was the \
                first time I built a complete end-to-end system from hardware to \
                desktop application.</p>
                <p>I connected an RFID reader via serial port using pySerial, set up \
                antenna systems for tag detection, and built the desktop interface \
                with PyQt. The moment I scanned a physical RFID tag and saw the UUID \
                appear on screen in real time, I felt a rush that is hard to describe.</p>
                <p>After graduating, I have been working for over five years as a backend \
                developer with Python, Django REST Framework, and FastAPI, building \
                scalable APIs, designing permission systems, and working with AWS services.</p>
                <p>Working at Tausand would be a nice opportunity to combine my backend \
                experience with software that connects to real instruments, something \
                I have not done since university and would enjoy getting back to.</p>
            """
        )
        self.verticalLayout_aboutme.addWidget(self.aboutMeLabel)

        self.verticalLayout.addWidget(self.aboutMeFrame)
